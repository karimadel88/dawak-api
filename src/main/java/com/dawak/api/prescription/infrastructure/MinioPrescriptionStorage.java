...
public class MinioPrescriptionStorage implements PrescriptionStorage {
    private static final Logger log = LoggerFactory.getLogger(MinioPrescriptionStorage.class);
    ...
    void verifyBucket() {
        try {
            ...
            log.info("Private MinIO prescription bucket ready bucket={}", bucket);
    ...
    public void write(String key, byte[] content) {
        try {
            ...
            log.info("Encrypted prescription object stored bucket={} object={} encryptedBytes={}",
                    bucket, objectName(key), encrypted.length);
    ...
    public byte[] read(String key) {
        try (var input = client.getObject(GetObjectArgs.builder().bucket(bucket).object(objectName(key)).build())) {
            ...
            byte[] content = encryption.decrypt(encrypted);
            log.info("Encrypted prescription object read bucket={} object={} encryptedBytes={} plaintextBytes={}",
                    bucket, objectName(key), encrypted.length, content.length);
            return content;
    ...
    public void delete(String key) {
        try {
            client.removeObject(RemoveObjectArgs.builder().bucket(bucket).object(objectName(key)).build());
            log.info("Encrypted prescription object deleted bucket={} object={}", bucket, objectName(key));
        }
    ...
    private ApiException unavailable(String message, Exception cause) {
        log.error("MinIO prescription storage failure bucket={} operationMessage={}", bucket, message, cause);
